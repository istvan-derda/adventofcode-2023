...
impl Hand {
    fn compute_hand_type(cards: Vec<Card>) -> HandType {
        ...
        let joker_count = cards_count.get(&JOKER).unwrap_or(&0).clone();
        cards_count.insert(JOKER, 0);

        let mut counts: Vec<u32> = cards_count.values().map(|v| v.clone()).collect();
        counts.sort();
        counts.reverse();
        counts[0].add_assign(joker_count);
        if counts[0] == 5 {
            FiveOfAKind
        } else if counts[0] == 4 {
            FourOfAKind
